...
const LOG_CAP = 400;
let job = null; // {id, target, method, status:"running"|"done"|"failed", log:[], exitCode, note}
let jobCounter = 0;

// opencode --version çalıştırmak ucuz değildir (npm launcher'ı eksik ikiliyi
// indirmeye kalkabilir). Bu yüzden tespit sonuçları önbelleğe alınır ve
// kurulum sürerken opencode ASLA yeniden çalıştırılmaz.
...
function execFileP(cmd, args, options = {}) {
  return new Promise((resolve) => {
    execFile(cmd, args, { timeout: 8000, windowsHide: true, ...options }, (err, stdout, stderr) => {
...
function versionOf(bin) {
  // Windows'ta npm shim'leri .cmd dosyasıdır; execFile bunları doğrudan
  // çalıştıramaz (EINVAL) — cmd.exe üzerinden, pencere açmadan çalıştır.
  if (process.platform === "win32") {
    return execFileP("cmd.exe", ["/d", "/s", "/c", `"${bin}" --version`], {
      windowsVerbatimArguments: true,
      timeout: 30000,
    });
  }
  return execFileP(bin, ["--version"], { timeout: 30000 });
}

async function detectCliFresh() {
  const isWin = process.platform === "win32";
  const candidates = [];

  // PATH üzerinde ara (Windows'ta "where" birden çok satır döndürür; .cmd tercih edilir)
  const which = await execFileP(isWin ? "where" : "which", ["opencode"]);
  if (!which.err) {
    const score = (p) => (p.endsWith(".cmd") ? 3 : p.endsWith(".exe") ? 2 : 1);
    const lines = which.stdout.split(/\r?\n/).map((s) => s.trim()).filter(Boolean);
    if (isWin) lines.sort((a, b) => score(b) - score(a));
    for (const line of lines) if (!candidates.includes(line)) candidates.push(line);
  }
...
async function detectCli(force = false) {
  // Kurulum sürerken opencode'u çalıştırmak yarım kalmış kurulumu tetikler;
  // bilinen son durumu döndür.
  if (!force && job && job.status === "running") {
    return cliCache.value || { installed: false, path: null, version: null };
  }
  if (!force) {
    if (cliCache.value && Date.now() - cliCache.at < CLI_CACHE_MS) return cliCache.value;
    if (cliDetectPromise) return cliDetectPromise; // eşzamanlı istekler tek tespiti paylaşır
  }
  cliDetectPromise = detectCliFresh();
  try {
    const value = await cliDetectPromise;
    cliCache = { at: Date.now(), value };
    return value;
  } finally {
    cliDetectPromise = null;
  }
}
...
async function fullStatus() {
  ...
  return {
    ...
    job: job
      ? { id: job.id, target: job.target, method: job.method, status: job.status, note: job.note, exitCode: job.exitCode, log: job.log.slice(-80) }
      : null,
  };
}
...
function runShell(commandString) {
  ...
  const isWin = process.platform === "win32";
  const child = isWin
    ? spawn("cmd.exe", ["/d", "/s", "/c", commandString], { windowsVerbatimArguments: true, windowsHide: true })
...
function startCliInstall(method) {
  ...
  let commandString;
  if (method === "npm") {
    // --allow-scripts: npm 11'in güvenlik özelliği opencode'un postinstall
    // betiğini engelliyor; betik engellenirse gerçek ikili dosya inmez ve
    // opencode her açılışta kendini yeniden kurmaya çalışır. Eski npm
    // sürümleri bu bayrağı tanımaz ama yalnızca uyarı verip devam eder.
    commandString = `npm install -g ${SOURCES.cli.npmPackage}@latest --allow-scripts=${SOURCES.cli.npmPackage}`;
  ...
  job = { id: ++jobCounter, target: "cli", method, status: "running", log: [], exitCode: null, note: null };
  ...
  child.on("close", async (code) => {
    job.exitCode = code;
    if (code === 0) {
      job.status = "done"; // önce işi bitmiş say ki tespit önbelleği atlamasın
      const cli = await detectCli(true);
      if (cli.installed) {
...
function startDesktopInstall() {
  ...
  job = { id: ++jobCounter, target: "desktop", method: "download", status: "running", log: [], exitCode: null, note: null };
